...
### Meeting Calendar and Hosting

The [W3C DDS calendar](https://www.w3.org/groups/cg/dds/calendar/) is the canonical public meeting calendar. Chairs schedule the recurring plenary on the first Thursday of each month from 14:00 to 15:00 UTC, invite the DDS Community Group, and make both the event and joining information visible to everyone. Agendas and minutes belong on the relevant W3C occurrence. Public attendance and subscription instructions are in [Open Calls](SUPPORT.md#open-calls).

Chairs maintain a stable public joining link and arrange hosting access so a designated co-host can run the call without depending on one organizer's attendance. The hosting provider, account, and recording tools may change; no particular Chair's account or Google configuration is a permanent requirement. Chairs document hosting changes, update public joining information, and test co-host access before relying on it.

For now, a separate chairs-only Google Calendar invitation manages the Google Meet link, advance co-host assignments, recording, and transcription. It is not the community's public calendar or invitation mechanism. Meet co-host permissions are separate from event-editing permissions. The Google and W3C events do not synchronize automatically, so schedule changes, cancellations, and joining-link changes must be reflected in both. A Chair without Google event-editing access coordinates changes with its organizer. Public announcements use the W3C calendar and [public mailing list](SUPPORT.md#mailing-list). Chairs who also subscribe to W3C may see duplicate entries.

Chairs must disclose recording, transcription, and intended YouTube and written-record publication in the event information and at the start of the call. Keep source recordings restricted to organizers until reviewed, and check storage access and automatic sharing settings. Confirm publication permissions and remove sensitive material before publishing reviewed video on YouTube with **Public** visibility and written records in the [open-calls repository](https://github.com/dds-wg/open-calls). Do not commit audio/video files to Git. Follow the [meeting-record publication workflow](https://github.com/dds-wg/open-calls#publishing-meeting-records), then link the published records from the specific W3C occurrence and announce them on the public mailing list.
